...
- **Cloudflare Pages "Clean URLs" strips `.html` with a 307**: with Clean URLs enabled, `GET /page.html` → 307 → `/page` (only the custom `_headers` are present; Pages serves both forms with 200). If the sitemap/canonical/internal links all use `.html` while the live server redirects to clean URLs, Google Search Console reports those pages as "Página de redirecionamento" and they won't index — while clean-URL variants crawl fine (the "some pages index, some don't" pattern). Diagnostic: `curl -s -o /dev/null -w '%{http_code} -> %{redirect_url}\n'` on each sitemap URL. Fix (ensinamentosdavida.com.br, 18/08/2026): switch sitemap.xml, rss.xml, all `<link rel=canonical>`/`og:url`, and internal hrefs to clean URLs — do NOT fight the server. Verify live after deploy.
