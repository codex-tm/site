...
7. **P2-1 resolvido (17/08/2026):** autor dos artigos agora é a Pessoa "Fernando" — JSON-LD `author` virou `Person` (com url pra /sobre.html) nos 5 artigos, byline visível "Por Fernando" no header de cada artigo, meta author atualizado, e a página `/sobre.html` foi criada com schema `AboutPage`+`Person` e entrou no sitemap. Decisão do dono: usar o nome real em vez da marca, conforme o Google recomenda pra E-E-A-T.
8. **P3-1 decidido (17/08/2026):** manter o bloqueio de bots de treino de IA gerenciado pelo Cloudflare (GPTBot, ClaudeBot, Google-Extended etc.). Decisão do dono: aparecer em respostas de IA não traz vantagem de visibilidade hoje; se um dia passar a trazer, reavaliar. Googlebot nunca foi afetado — busca Google funciona normal.
